# -*- coding: utf-8 -*-
"""
Created on Wed Nov 18 15:18:27 2020
...
if __name__ == "__main__":
    ...
    trials = 30
    start = time.time()
    #m_out = [np.zeros(num_sites),np.zeros(num_sites)]
    m_out = [[0]*num_sites]*trials
    for iteration in range(trials):
        print(iteration)
        real = []
        #real_count: data[day_index][hour][loc_id]
        for x in range(0, num_days):
            real.append(np.zeros((24, num_sites))) 
            
        real_count = np.array(real)
        noise_count = np.array(real)
        ...
        with concurrent.futures.ProcessPoolExecutor() as executor:
            ...
            for row in range(len(csv_reader)):
                ...
                if(line_count == 0):
                    ...
                    if(csv_reader.loc[row][1] != name):
                        #print("User # " + str(user_processing_no) )
            ...
            for result in concurrent.futures.as_completed(mylist):
                ...
                #print("remaining: " + str(users_remaining))
        ...
        pd.DataFrame(real_count[0]).to_csv("daylog.csv")
        # pd.DataFrame(real_count[1]).to_csv("day1.csv") 
        pd.DataFrame(noise_count[0]).to_csv("daylogDP.csv")
        ...
        #m = [np.zeros(num_sites),np.zeros(num_sites)]

        for row in n[0]:
            m_out[iteration] = m_out[iteration] + row
            
        #print(m_out[iteration])
        ds = pd.Series(m_out[iteration])        #change from last night to test
        ds = np.transpose(ds)
        #ds.to_csv("noisedsummary.csv")
        ...
        #noise_logs.append(m[iteration])
    
    #nl = noise_logs    
    nl = pd.DataFrame(m_out)
    nl = np.transpose(nl)
    nl.to_csv("noisedsummary.csv")
    """
